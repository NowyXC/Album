 ...
 *
 * 存放各类系统相关的方法
 * # 读取SIM卡序列号（Manifest.permission.READ_PHONE_STATE）
 * # 获取手机CPU序列号
 * # 获得电话管理实例对象
 * # 读唯一的设备ID(唯一的设备ID【GSM手机的IMEI】和【CDMA手机的 MEID】,如果获取不到返回一个默认字符串)（Manifest.permission.READ_PHONE_STATE）
 * # 获取运营商信息(三大运营商)
 * # 获取手机状态(0：无活动 1：响铃 2：待机)
 * # 获得手机方位 （Manifest.permission.ACCESS_FINE_LOCATION && Manifest.permission.ACCESS_COARSE_LOCATION）
 * # 获得设备的软件版本号(注：the IMEI/SV(software version) for GSM phones 不支持返回“not available”) （Manifest.permission.READ_PHONE_STATE）
 * # 获得手机号 （Manifest.permission.READ_SMS && Manifest.permission.READ_PHONE_STATE）
 * # 获得SIM卡提供的移动国家码和移动网络码.5或6位的十进制数字
 * # 服务商名称
 * # 获取国际移动用户GSM识别码 （Manifest.permission.READ_PHONE_STATE）
 * # 获取WindowManager对象
 * # 获取屏幕管理类
 * # 获得系统配置相符的线程池大小
 * # 获取震动器对象
 * # 获取手机IP地址
 * # 获取AndroidID
 * # 获取设备IMSI码 （Manifest.permission.READ_PHONE_STATE）
 * # 获取网络IP地址(优先获取wifi地址)
 * # 获取设备序列号
 * # 获取网络运营商
 * # 获取网络运营商名称
 ...
 * # 获取国家
 * # 获取系统版本
 * # 获取GSF序列号 (Manifest.permission.READ_GSERVICES)
 * # 获取蓝牙地址 (Manifest.permission.BLUETOOTH)
 * # Android设备物理唯一标识符
 * # 获取硬件信息
 * # 获取产品信息
 * # 获取设备信息
...
public class SystemUtil {
	...
	public static String getSysSIMSerialNum(Context content) {
		if (ActivityCompat.checkSelfPermission(content, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
			...
			Logger.t(TAG).i("SystemUtil-->>getSysSIMSerialNum", simSerialNumber);
	...
	public static String getSysCPUSerialNum() {
		...
		try {
			...
			e.printStackTrace();
			Logger.t(TAG).e("SystemUtil-->>getSysCPUSerialNum", e.getMessage().toString());
		}
		return cpuSerialNum;
	}
	...
	private static TelephonyManager getSysTelephonyManager(Context content) {
		...
		Logger.t(TAG).i("SystemUtil-->>getSysTelephonyManager", telephonyManager + "");
	...
	public static String getSysTelephoneSerialNum(Context content) {
		if (ActivityCompat.checkSelfPermission(content, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
			...
			Logger.t(TAG).i("SystemUtil-->>getSysTelephoneSerialNum", deviceId + "");
			return deviceId;
		}
	...
	public static String getSysCarrier(Context content) {
		...
		if (ActivityCompat.checkSelfPermission(content, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
			...
			Logger.t(TAG).i("SystemUtil-->>getSysCarrier", moblieType);
			return moblieType;
		}
	...
	public static Integer getSysPhoneState(Context context) {
		...
		Logger.t(TAG).i("SystemUtil-->>getSysPhoneState", callState + "");
		return callState;
	}
	...
	public static CellLocation getSysPhoneLoaction(Context context) {
		if (ActivityCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED
			...
			Logger.t(TAG).i("SystemUtil-->>getSysPhoneLoaction", cellLocation + "");
			return cellLocation;
		}
	...
	public static String getSysDeviceSoftVersion(Context context) {
		if (ActivityCompat.checkSelfPermission(context, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
			...
			Logger.t(TAG).i("SystemUtil-->>getSysDeviceSoftVersion", deviceSoftwareVersion + "");
	...
	public static String getSysPhoneNumber(Context context) {
		if (ActivityCompat.checkSelfPermission(context, Manifest.permission.READ_SMS) != PackageManager.PERMISSION_GRANTED
			...
			Logger.t(TAG).i("SystemUtil-->>getSysPhoneNumber", phoneNumber + "");
			return phoneNumber;
		}
	...
	public static String getSysSimCode(Context context) {
		...
		Logger.t(TAG).i("SystemUtil-->>getSysSimCode", code + "");
		return code;
	}
	...
	public static String getSysSimPrivatorName(Context context) {
		...
		Logger.t(TAG).i("SystemUtil-->>getSysSimPrivatorName", simOperatorName);
	...
	public static String getSysUserPhoneId(Context context) {
		if (ActivityCompat.checkSelfPermission(context, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
			...
			Logger.t(TAG).i("SystemUtil-->>getSysUserPhoneId", subscriberId);
			return subscriberId;
		}
	...
	public static DisplayMetrics getSysDisplayMetrics(Activity activity) {
		...
		Logger.t(TAG).i("SystemUtil-->>getSysDisplayMetrics", "获取屏幕管理对象为：" + displayMetrics);
	...
	public static Integer getSysDefaultThreadPoolSize() {
		...
		Logger.t(TAG).i("SystemUtil-->>getSysDefaultThreadPoolSize", availableProcessors + "");
	...
	public String getSysLocalIpAddress() {
		...
		try {
			...
			e.printStackTrace();
			Logger.t(TAG).e("SystemUtil-->>getSysLocalIpAddress", e.getMessage().toString());
		}
		Logger.t(TAG).i("SystemUtil-->>getSysLocalIpAddress", hostAddress);
		return hostAddress;
	}
	...
	public static String getIdentity(Context context){
		String szImei  = getSysTelephonyManager(context).getDeviceId();;//设备唯一码
		String mSdk = Build.VERSION.RELEASE;// 系统版本
		String mtyb = Build.BRAND;// 手机品牌
		String mtype = Build.MODEL; // 手机型号
		String identity = szImei + mSdk + mtyb + mtype;
		return identity;
